...
public class InternalSessionsController {
    ...
    public ResponseEntity<SessionResponse> createSession(@Valid @RequestBody CreateSessionRequest req) {
        List<AssetRef> assets = req.assets().stream().map(a ->
                new AssetRef(a.assetId(), a.s3Key(), a.contentType(), a.filename(), a.sizeBytes())
        ).toList();
    ...
    public ResponseEntity<?> getSession(@PathVariable UUID id) {
        ...
        if (s.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                    ProblemDetailFactory.of(HttpStatus.NOT_FOUND,
                            "Session not found",
    ...
    public ResponseEntity<?> getReport(@PathVariable UUID id) {
        ...
        if (s.get().state() != SessionState.REPORT_READY) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    ProblemDetailFactory.of(HttpStatus.CONFLICT,
                            "Report not ready",
        ...
        return get.findReport(new SessionId(id))
                ...
                .orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                        ProblemDetailFactory.of(HttpStatus.NOT_FOUND,
                                "Report missing",
                                "Session is REPORT_READY but no report row exists",
                                "REPORT_MISSING")));
    }
}
