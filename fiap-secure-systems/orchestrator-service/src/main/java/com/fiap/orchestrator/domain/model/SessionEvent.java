...
public record SessionEvent(
    ...
    public static SessionEvent initial(
            EventId id, SessionId sessionId, SessionState toState,
    ...
    public static SessionEvent transition(
            EventId id, SessionId sessionId,
            SessionState fromState, SessionState toState,
            Map<String, Object> payload, Instant occurredAt) {
        Objects.requireNonNull(fromState, "fromState (use initial() for null)");
        if (fromState == toState) {
            throw new IllegalArgumentException("transition must change state (got " + fromState + " → " + toState + ")");
        }
