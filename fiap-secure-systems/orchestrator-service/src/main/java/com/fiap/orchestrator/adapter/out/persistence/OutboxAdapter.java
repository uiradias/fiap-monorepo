...
public class OutboxAdapter implements OutboxPort {
    ...
    public void append(
        ...
        outbox.save(OutboxMessageEntity.create(
                UUID.randomUUID(), aggregateId.value(), destination, eventType, payload, now));
    }

    @Override
    public List<OutboxEntry> fetchUnpublished(int limit) {
        return outbox.findUnpublished(PageRequest.of(0, limit)).stream()
                .map(e -> new OutboxEntry(
                        e.getId(), new SessionId(e.getAggregateId()),
                        e.getDestination(), e.getEventType(), e.getPayload(),
                        e.getCreatedAt(), e.getAttempts(), e.getLastError()))
                .toList();
    }

    @Override
    public void markPublished(UUID id, Instant now) {
        OutboxMessageEntity e = outbox.findById(id)
