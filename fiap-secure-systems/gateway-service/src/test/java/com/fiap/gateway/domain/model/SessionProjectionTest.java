...
class SessionProjectionTest {
    ...
    private SessionEventLogEntry event(
            SessionState from, SessionState to, Instant t, Map<String, Object> p) {
    ...
    void applyEvent_advances_state() {
        SessionProjection p =
                SessionProjection.initial(SID, UID, SessionState.ASSETS_UPLOADED, T0)
                        .applyEvent(
                                event(
    ...
    void applyEvent_captures_errorCode_on_FAILED() {
        SessionProjection p =
                SessionProjection.initial(SID, UID, SessionState.ANALYZING, T0)
                        .applyEvent(
                                event(
                                        SessionState.ANALYZING,
                                        SessionState.FAILED,
                                        T0.plusSeconds(1),
                                        Map.of("errorCode", "MODEL_RATE_LIMITED")));
    ...
    void applyEvent_captures_reportId_on_REPORT_READY() {
        ...
        SessionProjection p =
                SessionProjection.initial(SID, UID, SessionState.ANALYSIS_COMPLETED, T0)
                        .applyEvent(
                                event(
                                        SessionState.ANALYSIS_COMPLETED,
    ...
    void applyEvent_keeps_latest_lastEventAt_on_late_arrival() {
        SessionProjection p1 =
                SessionProjection.initial(
                        SID, UID, SessionState.QUEUED_FOR_ANALYSIS, T0.plusSeconds(10));
        SessionProjection p2 =
                p1.applyEvent(
                        event(
                                SessionState.QUEUED_FOR_ANALYSIS,
                                SessionState.ANALYZING,
    ...
    void applyEvent_rejects_mismatched_session_id() {
        ...
        org.junit.jupiter.api.Assertions.assertThrows(
                IllegalArgumentException.class, () -> p.applyEvent(foreign));
    }
}
