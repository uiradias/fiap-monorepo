...
public class InMemoryFakes {
    ...
    public static class SessionRepoFake implements SessionRepositoryPort {
        ...
        @Override public Session insertIfAbsent(Session session) {
        ...
        @Override public Session save(Session session) {
            ...
            if (existing.version() != session.version() - 1) {
                throw new org.springframework.dao.OptimisticLockingFailureException(
                        "version mismatch (expected %d, got %d)".formatted(
                                existing.version(), session.version() - 1));
            }
            store.put(session.id().value(), session);
            return session;
        }
        @Override public void appendEvent(SessionEvent event) { events.add(event); }
    }
    ...
    public static class ReportRepoFake implements ReportRepositoryPort {
        ...
        @Override public Optional<AnalysisReport> findBySessionId(SessionId sessionId) {
            return Optional.ofNullable(store.get(sessionId.value()));
        }
        @Override public AnalysisReport save(AnalysisReport r) {
    ...
    public static class OutboxFake implements OutboxPort {
        ...
        @Override public void append(
                SessionId aggregateId, Destination destination, String eventType,
                Map<String, Object> payload, Instant now) {
            rows.add(new OutboxEntry(
                    UUID.randomUUID(), aggregateId, destination, eventType,
                    Map.copyOf(payload), now, 0, null));
        }
        @Override public List<OutboxEntry> fetchUnpublished(int limit) {
            return List.copyOf(rows);
        }
        @Override public void markPublished(UUID id, Instant now) {}
        @Override public void recordFailure(UUID id, String errorMessage) {}
    }
    ...
    public static class ProcessedResultsFake implements ProcessedResultsPort {
        ...
        @Override public boolean recordIfAbsent(JobId jobId, AnalysisStatus status, Instant at) {
    ...
    public static class TestClock implements Clock {
        ...
        public void advance(java.time.Duration d) { now = now.plus(d); }
    }
}
