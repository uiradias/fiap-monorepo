...
public interface OrchestratorClientPort {
    /**
     * Posts to orchestrator's POST /internal/sessions. The orchestrator's request body
     * uses an {@code assets} field containing one {@code AssetRequest} per asset
     * (assetId, s3Key, contentType, filename, sizeBytes) — pass the full {@link Asset}
     * records and let the adapter serialize the inner shape.
     */
