...
class AnalysisResultSqsConsumerTest {
    ...
    void setUp() {
        ...
        ContractValidator validator =
                new ContractValidator(
                        new File(System.getProperty("user.dir") + "/../infrastructure/contracts")
                                .getAbsoluteFile());
        Clock clock = Instant::now;

        consumer =
                new AnalysisResultSqsConsumer(
                        sqs, MAIN_URL, DLQ_URL, 1, mapper, validator, started, completed, failed,
                        clock);
    }
    ...
    void unknown_session_on_started_routes_message_to_dlq_and_deletes_from_main() throws Exception {
        ...
        doThrow(new UnknownSessionException(new SessionId(sessionId)))
                .when(started)
                .onStarted(any(JobId.class), any(SessionId.class));
                ...
                .sendMessage(
                        argThat(
                                (SendMessageRequest req) ->
                                        DLQ_URL.equals(req.queueUrl())
                                                && body.equals(req.messageBody())));
        verify(sqs, times(1))
                .deleteMessage(
                        argThat(
                                (DeleteMessageRequest req) ->
                                        MAIN_URL.equals(req.queueUrl())
                                                && "rh-1".equals(req.receiptHandle())));
    }
    ...
    void transient_failure_on_started_leaves_message_for_redelivery() throws Exception {
        ...
        // Transient failures: do NOT delete from main, do NOT push to DLQ — let visibility timeout
        // requeue.
    ...
    void unparseable_message_is_dropped_without_dlq_forwarding() {
        Message m =
                Message.builder().body("not-json").receiptHandle("rh-3").messageId("mid-3").build();
        ...
        verify(sqs, times(1))
                .deleteMessage(
                        argThat((DeleteMessageRequest req) -> "rh-3".equals(req.receiptHandle())));
        verify(sqs, never()).sendMessage(any(SendMessageRequest.class));
    }
