...
/**
 * Permanent error: the referenced session does not exist in the orchestrator's store.
 * Treated as a poison pill by message consumers — the message is forwarded to its DLQ
 * rather than left for visibility-timeout redelivery (which would never succeed).
 */
