...
public class AnalysisResultSqsConsumer {
    ...
    void handleOne(Message m) {
        ...
        try {
            ...
            log.warn(
                    "dropping unparseable/invalid analysis-results message: {}",
        ...
        try {
            ...
            log.warn(
                    "poison pill on analysis-results jobId={} status={}: {}",
                    body.get("jobId"),
                    body.get("status"),
                    poisonPill.getMessage());
            forwardToDlq(m);
            deleteSafely(m);
        } catch (Exception useCaseError) {
            log.error(
                    "use case failed for analysis-results jobId={} status={}; leaving for"
                            + " redelivery",
                    body.get("jobId"),
                    body.get("status"),
                    useCaseError);
    ...
    private void forwardToDlq(Message m) {
        try {
            sqs.sendMessage(
                    SendMessageRequest.builder().queueUrl(dlqUrl).messageBody(m.body()).build());
        } catch (Exception e) {
            log.warn(
                    "failed to forward poison-pill message {} to DLQ {}: {}",
                    m.messageId(),
                    dlqUrl,
                    e.getMessage());
        }
    ...
    private void deleteSafely(Message m) {
        try {
            ...
            log.warn(
                    "failed to delete analysis-results message {}: {}",
                    m.messageId(),
                    e.getMessage());
        }
    }
