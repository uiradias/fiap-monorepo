...
public class SessionEventLogRepositoryAdapter implements SessionEventLogRepositoryPort {
    ...
    public SessionEventLogRepositoryAdapter(
            SessionEventLogJpaRepository repo, ObjectMapper mapper) {
        this.repo = repo;
        this.mapper = mapper;
    }
    ...
    public boolean insertIfAbsent(SessionEventLogEntry entry) {
        String payloadJson;
        try {
            payloadJson = mapper.writeValueAsString(entry.payload());
    ...
    public List<SessionEventLogEntry> tailForSession(SessionId sessionId, int limit) {
        return repo.tailForSession(sessionId.value(), limit).stream()
                .map(
                        e -> {
                            Map<String, Object> p;
                            try {
                                p = mapper.readValue(e.payload, Map.class);
