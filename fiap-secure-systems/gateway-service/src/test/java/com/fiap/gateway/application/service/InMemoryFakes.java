...
public final class InMemoryFakes {
    ...
    public static final class FakeUsers implements UserRepositoryPort {
        ...
        @Override public User insertOrThrow(User u) {
    ...
    public static final class FakeRefreshTokens implements RefreshTokenRepositoryPort {
        ...
        public FakeRefreshTokens(Clock clock) { this.clock = clock; }
        public FakeRefreshTokens() { this(Instant::now); }

        @Override public RefreshToken insert(RefreshToken t) {
            byId.put(t.id(), t);
            byHash.put(t.tokenHash(), t.id());
            return t;
        }
        @Override public Optional<RefreshToken> findActiveByHash(String hash) {
        ...
        @Override public void revoke(RefreshTokenId id, Instant now) {
            RefreshToken t = byId.get(id);
            if (t != null) byId.put(id, t.revoke(now));
        }
        @Override public void revokeAllForUser(UserId userId, Instant now) {
            byId.replaceAll((id, t) -> t.userId().equals(userId) && t.revokedAt() == null ? t.revoke(now) : t);
        }
    }
    ...
    public static final class FakeBundles implements AssetBundleRepositoryPort {
        ...
        @Override public AssetBundle insert(AssetBundle b) { byId.put(b.id(), b); return b; }
        @Override public AssetBundle save(AssetBundle b) { byId.put(b.id(), b); return b; }
        @Override public Optional<AssetBundle> findById(BundleId id) { return Optional.ofNullable(byId.get(id)); }
    }
    ...
    public static final class FakeAssets implements AssetRepositoryPort {
        ...
        @Override public Asset insert(Asset a) { byId.put(a.id(), a); return a; }
        @Override public List<Asset> findByBundleId(BundleId bid) {
    ...
    public static final class FakeProjections implements SessionProjectionRepositoryPort {
        ...
        @Override public SessionProjection upsert(SessionProjection p) { byId.put(p.id(), p); return p; }
        @Override public Optional<SessionProjection> findById(SessionId id) { return Optional.ofNullable(byId.get(id)); }
    }
    ...
    public static final class FakeEventLog implements SessionEventLogRepositoryPort {
        ...
        @Override public synchronized boolean insertIfAbsent(SessionEventLogEntry e) {
            return byEventId.putIfAbsent(e.eventId(), e) == null;
        }
        @Override public synchronized List<SessionEventLogEntry> tailForSession(SessionId sid, int limit) {
    ...
    public static final class FakeS3 implements AssetStoragePort {
        ...
        @Override public String put(BundleId bundleId, AssetId assetId, String filename,
                                    ContentType contentType, long sizeBytes, InputStream body) {
            String key = "sessions/" + bundleId + "/" + filename;
            try { body.transferTo(java.io.OutputStream.nullOutputStream()); }
    ...
    public static final class FakeBroadcaster implements SessionEventBroadcastPort {
        public final List<SessionEventLogEntry> broadcasts = Collections.synchronizedList(new ArrayList<>());
        @Override public void broadcast(SessionEventLogEntry e) { broadcasts.add(e); }
    }

    public static final class FakeHasher implements PasswordHasherPort {
        @Override public PasswordHash hash(String pw) { return new PasswordHash("hash:" + pw); }
        @Override public boolean matches(String pw, PasswordHash h) { return h.value().equals("hash:" + pw); }
    }
    ...
    public static final class FakeTokens implements TokenIssuerPort {
        ...
        @Override public String issueAccessToken(UserId uid, Instant now) {
            String t = "access:" + uid + ":" + now.toEpochMilli();
            issued.put(uid, t);
            return t;
        }
        @Override public String generateRefreshTokenPlaintext() { return "refresh:" + (++refreshCounter); }
        @Override public String hashRefreshToken(String pt) { return "h(" + pt + ")"; }
        @Override public UserId verifyAccessToken(String jws) {
            ...
            return issued.entrySet().stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new com.fiap.gateway.domain.exception.InvalidTokenException("unknown token"));
        }
    }
}
