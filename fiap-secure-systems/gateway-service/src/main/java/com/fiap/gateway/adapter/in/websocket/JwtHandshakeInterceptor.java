...
public class JwtHandshakeInterceptor implements HandshakeInterceptor {
    ...
    public JwtHandshakeInterceptor(
            TokenIssuerPort tokens, SessionProjectionRepositoryPort projections) {
        this.tokens = tokens;
    ...
    public boolean beforeHandshake(
        ...
        UserId uid;
        try {
            uid = tokens.verifyAccessToken(token);
        ...
        SessionId sid;
        try {
            sid = new SessionId(UUID.fromString(path.substring(path.lastIndexOf('/') + 1)));
    ...
    public void afterHandshake(
            ServerHttpRequest req, ServerHttpResponse res, WebSocketHandler h, Exception ex) {}
        ...
        for (String part : q.split("&")) {
            ...
            if (eq > 0 && part.substring(0, eq).equals(name)) {
                return java.net.URLDecoder.decode(
                        part.substring(eq + 1), java.nio.charset.StandardCharsets.UTF_8);
            }
        }
        return null;
    }
