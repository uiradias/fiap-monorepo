...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail dupEmail(DuplicateEmailException e) {
        return ProblemDetailFactory.of(
                HttpStatus.CONFLICT, "Email already registered", e.getMessage(), "EMAIL_TAKEN");
    }
    ...
    public ProblemDetail sessionNotFound(SessionNotFoundException e) {
        return ProblemDetailFactory.of(
                HttpStatus.NOT_FOUND, "Session not found", e.getMessage(), "SESSION_NOT_FOUND");
    }

    @ExceptionHandler(ForbiddenException.class)
    public ProblemDetail forbidden(ForbiddenException e) {
        return ProblemDetailFactory.of(
                HttpStatus.FORBIDDEN, "Forbidden", e.getMessage(), "FORBIDDEN");
    }

    @ExceptionHandler(ReportNotReadyException.class)
    public ProblemDetail notReady(ReportNotReadyException e) {
        return ProblemDetailFactory.of(
                HttpStatus.CONFLICT, "Report not ready", e.getMessage(), "REPORT_NOT_READY");
    }
    ...
    public ProblemDetail illegalState(IllegalStateException e) {
        ...
        return ProblemDetailFactory.of(
                HttpStatus.CONFLICT, "Illegal state", e.getMessage(), "ILLEGAL_STATE");
    }

    @ExceptionHandler(InvalidPayloadException.class)
    public ProblemDetail badPayload(InvalidPayloadException e) {
        return ProblemDetailFactory.of(
                HttpStatus.BAD_REQUEST, "Invalid payload", e.getMessage(), "INVALID_PAYLOAD");
    }
}
