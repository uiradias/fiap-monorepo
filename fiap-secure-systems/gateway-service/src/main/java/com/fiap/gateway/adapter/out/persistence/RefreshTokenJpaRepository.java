...
public interface RefreshTokenJpaRepository extends JpaRepository<RefreshTokenEntity, UUID> {
    ...
    @Modifying
    @Query(
            "UPDATE RefreshTokenEntity r SET r.revokedAt = :now WHERE r.id = :id AND r.revokedAt IS"
                    + " NULL")
    int revokeById(@Param("id") UUID id, @Param("now") Instant now);

    @Modifying
    @Query(
            "UPDATE RefreshTokenEntity r SET r.revokedAt = :now WHERE r.userId = :uid AND"
                    + " r.revokedAt IS NULL")
