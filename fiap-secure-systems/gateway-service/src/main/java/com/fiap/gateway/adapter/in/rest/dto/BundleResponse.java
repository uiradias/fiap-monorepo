...
public record BundleResponse(
        UUID bundleId, String status, int assetCount, long totalBytes,
        Instant createdAt, Instant updatedAt, List<AssetResponse> assets) {
    public static BundleResponse of(AssetBundle b, List<AssetResponse> assets) {
        return new BundleResponse(
                b.id().value(), b.status().name(), b.assetCount(), b.totalBytes(),
                b.createdAt(), b.updatedAt(), assets);
    }
}
