...
class OutboxAdapterIT extends PostgresTestcontainersBase {
    ...
    void markPublished_excludes_from_unpublished() {
        SessionId sid = new SessionId(UUID.randomUUID());
        outbox.append(sid, OutboxPort.Destination.SQS_ANALYSIS_JOBS,
                "AnalysisJobRequested", Map.of("k", "v"), Instant.now());

        OutboxPort.OutboxEntry e = outbox.fetchUnpublished(50).stream()
                .filter(x -> x.aggregateId().equals(sid))
                .findFirst().orElseThrow();
        outbox.markPublished(e.id(), Instant.now());

        boolean stillUnpublished = outbox.fetchUnpublished(50).stream()
                .anyMatch(x -> x.id().equals(e.id()));
    ...
    void recordFailure_bumps_attempts() {
        ...
        OutboxPort.OutboxEntry refreshed = outbox.fetchUnpublished(50).stream()
