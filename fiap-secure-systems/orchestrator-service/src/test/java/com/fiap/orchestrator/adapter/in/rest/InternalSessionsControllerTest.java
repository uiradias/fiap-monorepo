...
class InternalSessionsControllerTest {
    ...
    @BeforeEach
    void setup() {
        InternalSessionsController controller = new InternalSessionsController(create, cancel, getUseCase);
    ...
    void post_internal_sessions_creates() throws Exception {
        ...
        mvc.perform(post("/internal/sessions")
    ...
    void get_report_returns_409_when_not_ready() throws Exception {
        Session s = Session.newSession(
                new SessionId(UUID.randomUUID()), new UserId(UUID.randomUUID()), 1, Instant.now());
